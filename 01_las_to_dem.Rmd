...
For DSM, we need normalized LAS files
...
sq <- c(0, 2, seq(5, plyr::round_any(ctg_norm$Max.Z, 5, ceiling), 5))
...
set_lidr_threads(1)
plan(multisession(workers = availableCores()/2))
dsm_5cm <- grid_canopy(ctg_norm, res = 0.05, algorithm = pitfree(sq, c(0, 1), subcircle = 0.025))
plan(sequential)
writeRaster(dsm_5cm, file.path(prod_dir, "dsm_5cm.tif"), overwrite = TRUE)
